...
def make_players_table(data, cur, conn):
    ...
    for player in data['squad']:
        name = player['name']
        position = player['position']
        birthyear = int(player['dateOfBirth'][:4]) # only need year
...
def nationality_search(countries, cur, conn):
    cur.execute("""
        ...
        WHERE Players.nationality IN ({})
    """.format(', '.join(['?']*len(countries))), countries)
...
def birthyear_nationality_search(age, country, cur, conn):
    year = 2023 - age
    cur.execute("""
        SELECT Players.name, Players.nationality, Players.birthyear
        FROM Players
        WHERE Players.nationality=? AND Players.birthyear<? 
    """, (country, year))
    results = cur.fetchall()
    return results
